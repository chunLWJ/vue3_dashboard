...
import {mapName, title} from '../data/index'
...
export default function initMap(elementId, _centerLinesMap, _defaultLinesMap) {
    ...
    [..._defaultLinesMap.values()].forEach(item => {
        ...
        const _value = getLineValue(item)
        let id = item.id,
            name = item.name,
            coords = [item.coordinate,centerLine ? centerLine.coordinate : []],
            value = _value
        locations.push({value: item.coordinate.slice(0, 2).concat([2, 2,_value])})
        lines.push({id, name, coords, value})
    });
    [..._centerLinesMap.values()].forEach(item => {
        const _value = getLineValue(item)
        locations.push({
            value: item.coordinate.slice(0, 2).concat([2, 2,_value])
        })
    });
    defaultLinesMap = _defaultLinesMap
